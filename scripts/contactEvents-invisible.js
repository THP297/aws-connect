...
import { SingleTon } from "./my-js/call/singleton.js";
...
export default function (contact) {
  ...
  logInfoMsg(
    "Contact attributes are " + JSON.stringify(contact.getAttributes())
  );
  ...
  function handleContactIncoming(contact) {
    ...
    if (contact) {
      ...
      logInfoEvent(
        "[contact.onIncoming] Contact is incoming. Null contact passed to event handler"
      );
    }
  }
  ...
  function handleContactConnecting(contact) {
    ...
    if (contact) {
      logInfoEvent(
        "[contact.onConnecting] Contact is connecting. Contact state is " +
          contact.getStatus().type
      );
      // document.getElementById("answerDiv").classList.add("glowingButton");
    } else {
      logInfoEvent(
        "[contact.onConnecting] Contact is connecting. Null contact passed to event handler"
      );
    }
  }
  ...
  function handleContactConnected(contact) {
    ...
    if (contact) {
      ...
      localStorage.removeItem("connectCallTime");
  ...
  function handleContactDestroyed(contact) {
    ...
    if (contact) {
      logInfoEvent(
        "[contact.onDestroy] Contact is destroyed. Contact state is " +
          contact.getStatus().type
      );
      ...
      logInfoEvent(
        "[contact.onDestroy] Contact is connecting. Null contact passed to event handler"
      );
    }
  }
}
